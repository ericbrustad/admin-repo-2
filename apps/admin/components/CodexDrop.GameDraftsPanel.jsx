...
export default function CodexDropGameDraftsPanel({
  ...
  useEffect(() => {
    ...
    setTitle(startTitle);
    setSlug(startSlug);
  }, [current?.id, current?.slug, current?.channel, current?.title]);
  ...
  const handleCloseAndSave = useCallback(async () => {
    ...
    if (current) {
      const nextGame = { ...current, title: title || current.title, slug: slug || current.slug };
      await saveDraft(nextGame, { title: nextGame.title, slug: nextGame.slug });
      enriched = enrichGameWithDraft(nextGame);
      const channel = normalizedChannel(enriched);
      if (enriched.id != null) {
        nextValue = `id:${enriched.id}::${channel}`;
      } else if (enriched.slug) {
        nextValue = `slug:${enriched.slug}::${channel}`;
      }
    }
    ...
    await reload();
    if (typeof onChange === 'function' && current && nextValue) {
      onChange(nextValue, enriched || current);
    }
  }, [current, onChange, onCloseAndSave, reload, slug, title, value]);
